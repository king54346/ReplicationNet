...
class Engine:
    ...
    def __init__(self, config: EngineConfig):
        ...
        # 6. CUDA Graph:创建 GraphRunner 优化推理性能
    ...
    # 通信初始化，单卡/pynccl 模式:使用 gloo 后端,避免 NCCL 的 GPU 内存占用
    # 多卡 NCCL 模式:GPU 通信用 nccl(all-reduce),CPU 同步用 gloo()
    # 时间 ↓
    # CPU:
    #  ┌────────────────────────────────────────┐
    #  │ Gloo: broadcast meta / barrier          │
    #  └────────────────────────────────────────┘
    #                 │
    # GPU:
    #  ┌────────────────────────────────────────┐
    #  │ NCCL TP: all-reduce QKV / logits        │
    #  │ NCCL PP: send / recv activation         │
    #  │ NCCL DP: all-reduce gradients / stats   │
    #  └────────────────────────────────────────┘
    #                 │
    # CPU:
    #  ┌────────────────────────────────────────┐
    #  │ Gloo: finished flags / scheduler sync   │
    #  └────────────────────────────────────────┘
    def _init_communication(self, config: EngineConfig) -> torch.distributed.ProcessGroup:

        if config.tp_info.size == 1 or config.use_pynccl:
            # 单卡或使用 pynccl 的情况，直接用 gloo 后端在 CPU 上初始化通信，所有通信都在 CPU 上进行，避免 NCCL 的 GPU 内存占用和潜在的跨设备通信问题
            ...
            # 多卡且不使用 pynccl 的情况，使用 nccl 后端在 GPU 上初始化通信，tp_cpu_group 是一个新的 gloo 后端的进程组，用于在 CPU 上同步所有 TP rank
            torch.distributed.init_process_group(
                backend="nccl",
                rank=config.tp_info.rank,
                world_size=config.tp_info.size,
    ...
    def _sync_get_memory(self) -> Tuple[int, int]:
        ...
        # 所有 TP rank 报告“我现在还剩多少 GPU 显存”，然后大家取一个“全 TP 都能接受的统一值”
    ...
    def forward_batch(self, batch: Batch, args: BatchSamplingArgs) -> ForwardOutput:
        ...
        for req in batch.reqs:
            req.complete_one()
        # 使用 Sampler 从 logits 生成下一个 token
        next_tokens_gpu = self.sampler.sample(logits[: batch.size], args).to(torch.int32)
        # 将结果异步拷贝到 CPU,使用 Event 同步
