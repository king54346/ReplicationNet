...
def tokenize_worker(
    *,
    ...
    # 它从 recv_listener 拉取一批消息（如果队列还有，就凑到 local_bs 为止），用于提高吞吐
    # 调用 detokenize_manager.detokenize(...)，把 token 还原成文本；把结果包成 UserReply 或 BatchFrontendMsg，发给前端 send_frontend
    # 调用 tokenize_manager.tokenize(...)，把文本转成 token；把结果包成 UserMsg 或 BatchBackendMsg，发给后端 send_backend
    # todo 多模态tokenizer
    try:
        while True:
